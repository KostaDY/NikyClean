...
# For each ticker defined in Excel:
#	1.	Downloads recent daily closes from Yahoo Finance.
#	2.	Converts daily % change into 3 discrete states:
#	•	"1" → ≤ −1%
#	•	"2" → between −1% and +1%
#	•	"3" → ≥ +1%
#	3.	Maintains a rolling buffer of the last 199 trading-day states.
#	4.	Builds:
#	•	Order-2 Markov model
#	•	Order-3 Markov model
#	5.	Predicts tomorrow’s state.
#	6.	Scores yesterday’s prediction against today’s factual outcome.
#	7.	Updates cumulative prediction accuracy matrix.
#	8.	Writes everything into markov.xlsx.
#	9.	Persists state in markov.csv.

#This is a self-evolving daily predictive Markov system.
...
# -1%, +1%] → "2"
# ≥ +1% → "3"
# So price becomes a symbolic sequence like# 
# 2 2 1 3 2 2 3 1 ...
# State Buffer: 199 digits 


# It is actually quite carefully engineered. Let me break it down structurally and conceptually.
...
# 2️⃣ State Buffer (Very Important)

# It keeps:

# DIGITS = 199 characters

# Stored youngest-first:

# newest_digit + previous_digits[:-1]

# So this is a cyclic rolling state memory.

# This is not recalculated from scratch every day.
# It evolves.

# That’s architecturally important.

# ⸻

# 3️⃣ Markov Model Construction

# Inside:

# markov_predict(digits, order)

# It:
# 	1.	Reverses to chronological order.
# 	2.	Builds transition count matrix.
# 	3.	Normalizes to transition probabilities.
# 	4.	Extracts current state.
# 	5.	Computes probability of next digit.

# Prediction = argmax of:
...
# Order-2 uses last 2 digits.
# Order-3 uses last 3 digits.
...
# 🔷 Daily Update Logic (Very Clever)

# It avoids double counting on same trading day.

# If today’s trading date != LAST_FACT_DATE:
# 	1.	Score yesterday’s prediction
# 	2.	Update accuracy matrix
# 	3.	Push factual digit into buffer
# 	4.	Save new LAST_FACT_DATE

# This makes it:

# ✔ idempotent
# ✔ safe across multiple runs per day
...
# It tracks a 3×3 confusion matrix flattened:
...
# Predicted X → Factual Y

# Then converts counts → percentages.

# This gives empirical calibration quality.
...
# So this script is incremental, not stateless.

# That’s the key architectural feature.
...
# Reads:
# 	•	Named range TICKERS from sheet M_Config

# Writes:
# 	•	Results to sheet M_Results

# Formats probability columns as %.

# Then:

# subprocess.run(["open", str(XLSX)])

# On macOS this opens Excel automatically.

# Very consistent with your workflow architecture.
...
# A daily online-learning symbolic Markov predictor for discretized stock returns, with persistent memory and empirical performance tracking.

# It is NOT:
# 	•	A stationary distribution engine
# 	•	A theoretical Markov study

# It IS:
# 	•	A practical predictive engine
# 	•	Incrementally evolving
# 	•	Accuracy-scored

# ⸻

# 🔷 Structural Observations (Technical)

# ✔ Good design:
# 	•	Safe backup of CSV before update
# 	•	Guard against double daily updates
# 	•	Validates digit buffer integrity
# 	•	Handles multi-index yfinance edge case

# ⚠ Notable limitations:
# 	•	No Laplace smoothing (zero rows possible)
# 	•	No probability calibration
# 	•	No confidence intervals
# 	•	No regime detection
# 	•	No volatility normalization

# But structurally clean.

# ⸻

# 🔷 In One Sentence

# This script implements a persistent, daily-updating, multi-order Markov chain predictor for stock direction states and logs both predictions and empirical accuracy into Excel.

# ⸻

# If you want, next we can:
# 	•	Analyze statistical stability of order-3 vs order-2
# 	•	Add Laplace smoothing
# 	•	Add Brier score
# 	•	Add entropy of prediction
# 	•	Or evaluate whether the accuracy matrix drifts toward uniformity
...
XLSX = Path("markov_A.xlsx")     # presentation workbook (also contains M_Config)
